...
local LibraryLoaded, Compkiller = pcall(function()
    local function notify(msg)
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Sheluvsblakefrfrv2 x Lubiebf4",
                Text = msg,
                Duration = 5
            })
        end)
    ...
    return loadstring(game:HttpGet(
        "https://raw.githubusercontent.com/sheluvsblakefrfr/GUI/refs/heads/main/GUI.lua"))()
end)
...
local Config = {
    ...
    -- Vehicle Tuner (Integrated from P2 PREM.LUA)
    VehicleTuner_Enabled = false,
    VehicleConfig = {
        FrontGrip = 85,
        RearGrip = 85,
        FrontDriftGrip = 60,
    ...
    -- Heli God Mode (Integrated from P2 PREM.LUA)
    HeliGodMode_Enabled = false,
    HeliConfig = {
        StabilityMult = 1.8,
        PIDBoost = 1.6,
        CollectiveBoost = 1.5,
...
-- =====================================================
-- BRM5 Vehicle Tuner & Helicopter God Mode Module (Integrated)
-- =====================================================
...
MOD.GetActiveController = function()
    local cs = getgenv()._CachedControllerService or ControllerService
...
MOD.ApplyVehicleTune = function(tune)
    ...
    local cfg = Config.VehicleConfig
...
-- Auto-Apply Loop
task.spawn(function()
    while getgenv().Blackhawk_Running do
        task.wait(1.5)
        pcall(function()
            if not MOD or not MOD._loopRunning then return end
            
            local ctrl = MOD.GetActiveController()
            if ctrl and ctrl._tune then
                if Config.VehicleTuner_Enabled and (ctrl._throttle ~= nil) then
                    MOD.SaveVehicleTune(ctrl._tune)
                    MOD.ApplyVehicleTune(ctrl._tune) 
                elseif not Config.VehicleTuner_Enabled and next(MOD.OriginalTune) ~= nil then
                    MOD.RestoreVehicleTune(ctrl._tune)
                end

                if Config.HeliGodMode_Enabled and (ctrl._PIDs ~= nil or ctrl._autoHover ~= nil) then
                    MOD.SaveHeliTune(ctrl)
                    MOD.ApplyHeliGodMode(ctrl) 
                elseif not Config.HeliGodMode_Enabled and next(MOD.OriginalHeliTune) ~= nil then
...
local function BulkScan(retry) -- High performance scanning engine
    ...
    getgenv()._CachedControllerService = ControllerService
...
-- (GetEntityType and IsTeammate defined below at lines ~1022/1041 â€” removed duplicate here)
...
-- Projectile lead and bullet drop calculation (O7: Optimized â€” single velocity magnitude check)
...
local function SolveLead(sourcePos, targetPos, targetVelocity, bulletSpeed, gravity)
    ...
    if Config.BulletDrop then
        ...
        -- 3D distance would inflate time when muzzle is above/below target â†’ dot too high
...
local function GetTargetVelocity(actor)
    ...
    -- Check if actor is seated in a vehicle â€” use vehicle velocity
...
local function IsTeammate(actor, cachedLocalSquad)
    ...
    -- In BRM5, by default all maps except strictly PVP are cooperative maps (PvE).
    -- If it's a real player and WE ARE NOT on a PVP map -> THEY ARE WITH US.
    ...
    -- Rule 2: PvP Maps -> We skip checking Roblox teams because they are bugged in BRM5.
    -- In PVP modes (including ZMP Survival), everyone WHO IS NOT IN YOUR SQUAD is treated as an enemy.

    -- Rule 3: ClientService Squad Check (Reliable party check from the main menu)
...
local function UpdateCombat(frameId)
    ...
    if equippedController then
        ...
        if Config.RageMode and ActorManager.SelectedTarget_RB and equippedController._firearm
            ...
            -- F1: Don't use bare return â€” it exits entire UpdateCombat
            if not targetActor or not targetActor.Alive or targetActor.Downed then
                ...
                elseif (Config.Rage_AutoFire or mousePressed) and (now - lastShot >= fireDelay) then
                    ...
                    -- F1: Don't return on no ammo â€” just skip shooting
                    if hasAmmo then
        ...
        -- (Dead safety block removed â€” was no-op)
...
DetectMapType = function()
    ...
    -- Special case for bugged Zombie mode IDs (ZMP / ZME share 4747446334)
    if currentId == 4747446334 or currentId == 84460047957624 then
        -- NEW, RELIABLE METHOD FOR ZOMBIE MAPS:
        -- MaxPlayers limit turned out to be completely bugged since both maps set 30-slot servers.
        -- So we check if there is a distinct PvE or PvP folder in Workspace generated by the engine.
        ...
        -- Fallback only in case of a space loading error (rare)
    ...
    -- Evidence from the PvE/PvP folder was primary, here WorldService acts as a certainty
    ...
    -- Keep only if something was definitely found (otherwise it leaves the previous name)
...
local function SetAllVisible(drawings, espData, visible)
    ...
    -- Highlight is a Roblox Instance â€” needs pcall (Plugin security context)
...
local function UpdateESPForActor(uid, espData, frameId, cachedCfg, localSquad)
    ...
    -- MODE: Simple ESP (BoxHandleAdornment) â€” P2: removed pcall closures
    if cachedCfg.SimpleESP then
        ...
        -- Emergency hide if player uses traditional 2D ESP while the loop still has a Simple ESP Box rendered from the previous frame:
    ...
    -- Update Highlight (pcall required â€” Roblox Instance security context)
    if espData.Highlight then
        pcall(function()
    ...
    if cachedCfg.UseBoxESP then
        if cachedCfg.BoxStyle == "Corner" then
            ...
            -- Full box style â€” hide corner lines using cached flag
...
task.spawn(function()
    ...
    mt.__index = newcclosure(function(self, key)
        ...
        -- P8: Direct call without pcall (massive perf gain â€” fires thousands of times/frame)
...
-- (Duplicate hook removed â€” SetupActorHooks already wraps ActorClass.Update in pcall)
...
pcall(function()
    loadNotify = Notifier.new({
        Title = "Sheluvsblakefrfrv2 x Lubiebf4",
        Content = "Initializing...\nNew Discord by Sheluvsblakefrfrv2 x nickcolaaaa",
        Duration = math.huge,
...
local successWindow, errWindow = pcall(function()
    Window = Compkiller.new({
        Name = "Sheluvsblakefrfrv2 x Lubiebf4",
        Keybind = "LeftAlt",
...
if not successWindow or not Window then
    warn("[BRM5] Error creating GUI window: " .. tostring(errWindow))
    print("[BRM5] Attempting to continue without GUI...")
    return
end
...
Window:Update({
    ...
    ExpireDate = "PREMIUM",
    WindowName = "Sheluvsblakefrfrv2 x Lubiebf4",
    Logo = Compkiller.Logo
})

-- Watermark HUD (Top-Right Corner)
local WM = Window:Watermark()
WM:AddText({ Icon = "zap", Text = "Sheluvsblakefrfrv2 x Lubiebf4" })
...
-- HOME TAB (Welcome & Info) â€” Premium Layout
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
-- â”€â”€ LEFT COLUMN: Welcome + Features â”€â”€
local S_Welcome = HomeTab:DrawSection({ Name = "Welcome", Position = "left" })

S_Welcome:AddParagraph({
    Title = '<font color="' ..
    GetRGB(Compkiller.Colors.Risky) ..
    '">⚡</font>  Sheluvsblakefrfrv2 x Lubiebf4  <font color="' .. GetRGB(Compkiller.Colors.Toggle) .. '">v2.1</font>',
...
S_Welcome:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">â™¦</font>  Credits',
    Content =
        'Developers: <font color="' ..
        GetRGB(Compkiller.Colors.Highlight) ..
        '">Sheluvsblakefrfrv2 x Lubiebf4</font>\nUI Framework: <font color="' ..
        GetRGB(Compkiller.Colors.Highlight) .. '">Compkiller</font> by 4lpaca'
})

S_Welcome:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">ðŸ’¬</font>  New Discord',
    Content = 'New Discord Made By <font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">Sheluvsblakefrfrv2 x nickcolaaaa</font>'
})

local S_Features = HomeTab:DrawSection({ Name = "Features", Position = "left" })

S_Features:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">ðŸŽ¯</font>  Combat',
    Content =
        '<font color="' ..
        GetRGB(Compkiller.Colors.SwitchColor) ..
        '">Silent Aim  Â·  Ragebot  Â·  No Recoil\nNo Spread  Â·  Custom RPM  Â·  Spinbot</font>'
})

S_Features:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">ðŸ‘</font>  Visuals',
    Content =
        '<font color="' ..
        GetRGB(Compkiller.Colors.SwitchColor) ..
        '">Box ESP  Â·  Corner ESP  Â·  Highlight\nHealth Bars  Â·  Tracers  Â·  Name Tags</font>'
})

S_Features:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">ðŸš€</font>  Movement',
    Content =
        '<font color="' ..
        GetRGB(Compkiller.Colors.SwitchColor) ..
        '">Vehicle Fly  Â·  Speed Hack  Â·  Noclip\nHitbox Expander  Â·  Vehicle Teleport</font>'
})

-- â”€â”€ RIGHT COLUMN: Changelog + Server â”€â”€
local S_Changelog = HomeTab:DrawSection({ Name = "Changelog", Position = "right" })

S_Changelog:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Risky) .. '">â˜…</font>  v3.0 â€” Security Update',
...
local KeybindsInfo = S_Server:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">âŒ¨</font>  Keybinds',
    Content =
        '<font color="' ..
        GetRGB(Compkiller.Colors.Risky) ..
        '">LeftAlt</font> â€” Toggle Menu\n<font color="' ..
        GetRGB(Compkiller.Colors.Risky) ..
        '">V</font> â€” Character Fly\n<font color="' .. GetRGB(Compkiller.Colors.Risky) .. '">X</font> â€” Vehicle Fly'
})
...
getgenv()._UpdateKeybindDisplay = function()
    pcall(function()
        ...
        local text = '<font color="' ..
            GetRGB(Compkiller.Colors.Risky) .. '">' .. tostring(k.Menu) .. '</font> â€” Toggle Menu'
            .. '\n<font color="' ..
            GetRGB(Compkiller.Colors.Risky) .. '">' .. tostring(k.CharFly) .. '</font> â€” Character Fly'
            .. '\n<font color="' ..
            GetRGB(Compkiller.Colors.Risky) .. '">' .. tostring(k.VehFly) .. '</font> â€” Vehicle Fly'
...
S_Server:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">ðŸ’¡</font>  Tip',
    Content = '<font color="' ..
    GetRGB(Compkiller.Colors.SwitchColor) .. '">Right-click any toggle to assign\na custom keybind to it.</font>'
})

S_Server:AddParagraph({
    Title = '<font color="' .. GetRGB(Compkiller.Colors.Highlight) .. '">ðŸŒ</font>  Server',
...
S_ESP:AddSlider({
    ...
    Flag = "ESP_MaxDist",
    Callback = function(v)
        -- Convert meters to studs for internal use (1m â‰ˆ 3.57 studs)
...
local charFlyKeybind = game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
...
table.insert(getgenv().BlackhawkESP_Connections, charFlyKeybind)
...
-- Flow: player interacts with door â†’ server creates LockPickController â†’ actor.LockPick = true
...
-- Vehicle Tuning Section (Integrated)
local S_VehTuning = VehicleTab:DrawSection({ Name = "Vehicle Tuning", Position = "left" })

S_VehTuning:AddToggle({
    Name = "Enable Tuner",
    Flag = "Veh_TuningEnabled",
    Default = Config.VehicleTuner_Enabled,
    Callback = function(v)
        Config.VehicleTuner_Enabled = v
        if not v then
            local ctrl = MOD.GetActiveController()
            if ctrl and ctrl._tune then MOD.RestoreVehicleTune(ctrl._tune) end
        end
    end
})

S_VehTuning:AddSlider({
    Name = "Acceleration Factor",
    Flag = "Veh_AccelFactor",
    Default = Config.VehicleConfig.AccelerationFactor,
    Min = 0.1,
    Max = 5,
    Precise = 1,
    Callback = function(v)
        Config.VehicleConfig.AccelerationFactor = v
    end
})

S_VehTuning:AddSlider({
    Name = "Grip (Front/Rear)",
    Flag = "Veh_Grip",
    Default = Config.VehicleConfig.FrontGrip,
    Min = 20,
    Max = 200,
    Callback = function(v)
        Config.VehicleConfig.FrontGrip = v
        Config.VehicleConfig.RearGrip = v
    end
})

-- Heli God Mode Section
local S_HeliGod = VehicleTab:DrawSection({ Name = "Heli God Mode", Position = "left" })

S_HeliGod:AddToggle({
    Name = "Enable Heli God",
    Flag = "Veh_HeliGodEnabled",
    Default = Config.HeliGodMode_Enabled,
    Callback = function(v)
        Config.HeliGodMode_Enabled = v
        if not v then
            local ctrl = MOD.GetActiveController()
            if ctrl then MOD.RestoreHeliTune(ctrl) end
        end
    end
})
...
S_HeliGod:AddSlider({
    Name = "Stability Mult",
    Flag = "Veh_HeliStability",
    Default = Config.HeliConfig.StabilityMult,
    Min = 1,
    Max = 2,
    Precise = 1,
    Callback = function(v)
        Config.HeliConfig.StabilityMult = v
    end
})
...
S_HeliGod:AddSlider({
    Name = "PID Boost",
    Flag = "Veh_HeliPID",
    Default = Config.HeliConfig.PIDBoost,
    Min = 1,
    Max = 2.5,
    Precise = 1,
    Callback = function(v)
        Config.HeliConfig.PIDBoost = v
    end
})

S_HeliGod:AddToggle({
    Name = "Force Auto-Hover",
    Flag = "Veh_HeliAutoHover",
    Default = Config.HeliConfig.ForceAutoHover,
    Callback = function(v)
        Config.HeliConfig.ForceAutoHover = v
    end
})
...
S_Settings:AddParagraph({
    Title = "Credits",
    Content = "Made by Sheluvsblakefrfrv2 x Lubiebf4 | Compkiller UI by 4lpaca"
})

S_Settings:AddParagraph({
    Title = "New Discord",
    Content = "New Discord Made By Sheluvsblakefrfrv2 x nickcolaaaa"
})
